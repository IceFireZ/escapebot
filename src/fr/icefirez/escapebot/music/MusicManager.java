	...
	public void loadTrack(final TextChannel channel, final String source) {
		...
		manager.loadItemOrdered(player, source, new AudioLoadResultHandler() {
			...
			public void trackLoaded(AudioTrack track) {
				channel.sendMessage("Ajout de la musique " + track.getInfo().title + ".").queue();
			...
			public void noMatches() {
				channel.sendMessage("La musique " + source + " n'a pas été trouvé.").queue();
			}

			@Override
			public void loadFailed(FriendlyException exception) {
				channel.sendMessage("Impossible de jouer la musique (Raison :" + exception.getMessage() + ")").queue();
			}
		});
	}
